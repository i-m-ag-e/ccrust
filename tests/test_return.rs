...
fn test_return() {
    ...
    let expected_ec = process::Command::new("tests/files/output/test_return_gcc")
        .status()
        .unwrap()
        .code()
        .unwrap();

    Command::new("tests/files/output/test_return")
        .assert()
        .code(expected_ec);
    fs::remove_file("tests/files/test_return.s").unwrap();
}
